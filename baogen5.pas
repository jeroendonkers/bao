...
// the same as baogen4, but the
// opening positions are generated at every generation,
// thus simulating a varying environment.
// - this means that all chromosomes have to be tested at
// every generation.
...
procedure test(var chr: TIchromosome; depth: integer; poslist: TBAoPositionList;
             maxmove: integer);
...
begin
   set_eval(parama,chr);
   chr.fitness:=0;
   for i:=1 to length(poslist) do begin
    pos := poslist[i];
    game_score := -99999;
    rootplay := pos.player;
    num:=0;
    ...
    repeat
     ...
     if num>maxmove then break;
   until endofgame(pos) or (num>maxmove);

   if endofgame(pos) then begin
     if checkwinner(pos)=rootplay then begin
        write('!');
        inc(chr.fitness);
     end else write('X');
   end else begin
     if checkmost(pos)=rootplay then begin
        write('!');
        inc(chr.fitness);
     end else write('X');
   end;
...
begin
  ...
  if (high(params)>=6) then begin
     numpos := strtoint(params[6]);
  end;
  depthpos:=10;
  if (high(params)>=7) then begin
     depthpos := strtoint(params[7]);
  end;
  ...
  o_eval := getevaluator(opp_eval);
   ...
   if not FileExists('result.txt') then begin
      ...
      rewrite(log);
      Writeln(log,'Genexp 5: search depth '+inttostr(depth)+
...
//   disable_hash;
   ...
   repeat
      ...
      create_randposlist(poslist,depthpos+ (generation mod 2), numpos);
      repeat
         Write('chr '+inttostr(nr)+': ');
         test(pop.chr[nr],depth,poslist,moves);
         store_population(pop,generation,nr);
...
begin
   registercommand(@comm_gen,'genexp5');
end.
